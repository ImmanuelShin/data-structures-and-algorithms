...
const raisedToTheThird = (arr) => {
  const tempArr = [];
  arr.forEach(element => tempArr.push(Math.pow(element, 3)));
  return tempArr;
};
...
const forEachTwoToThe = (arr) => {
  const tempArr = [];
  arr.forEach(e => tempArr.push(Math.pow(2, e)));
  return tempArr;
};
...
const charCode = (arr) => {
  const array = arr.map(e => e.charCodeAt(0));
  return array;
};
...
const extractAbilities = (arr) => {
  const array = arr.map(e => e.ability.name);
  return array;
};
