...
const sortByChildren = (charArray) => {
  return charArray.sort((a, b) => (a.children.length === b.children.length) ? a.name.localeCompare(b.name): a.children.length - b.children.length);
};
...
const isCapitalized = (str) => {
  return str.replace(/\W/g, ' ').split(' ').filter(word => /^[A-Z]/.test(word));
};
