...
const listFoods = (recipe) => {
  const result = recipe.ingredients.map(string => {
...
const splitFoods = (recipe) => {
  const result = recipe.ingredients.map(str => str.split(' ').slice(2).join(' ')).flat(Infinity);
  return result;
};
...
const stepActions = (recipe) => {
  const result = recipe.steps.map(step => step.split(' ').shift());
  return result;
};
...
const totalSumCSV = (str) => {
  const total = str.split(',').reduce((a, c) => a + Number(c), 0);
  return total;
};
...
const extractVowels = (str) => {
  return [str.replace(/[aeiou]/gi, ''), str.replace(/[bcdfghjklmnpqrstvwxyz-]/gi, '').split(' ').join('').split('').sort().join('')]; // lol
};
